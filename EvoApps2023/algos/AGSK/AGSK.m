function [res] = AGSK(fhd,d,max_nfes,lb,ub)
ConvDisp=0;
val_2_reach=1.00e-8;
problem_size = d;
Runs = 1;
...
for n=0:35
    RecordFEsFactor(n+1) = round(problem_size^((n/10)-3.5)*max_nfes);
end
    ...
    for func = 1
        ...
        for run_id = 1 : Runs
            ...
            while nfes < max_nfes
                ...
                Gained_Shared_Junior = boundConstraint_AGSK(Gained_Shared_Junior, pop, lu);
                Gained_Shared_Senior = boundConstraint_AGSK(Gained_Shared_Senior, pop, lu);
                ...
                [m,~] = size(ui); children_fitness = zeros(m,1);
                for i=1:m
...
res.progress_values = allerrorvals;
