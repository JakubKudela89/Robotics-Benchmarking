function res = cmaes(fhd_0,D,max_nfes,VarMin,VarMax)

%% Problem Settings

fhd = @(x) fhd_0(x) + (min(min(x),VarMin) -VarMin)^2+ (max(max(x),VarMax) -VarMax)^2; % add quadratic penalty for samples outsize the domain
